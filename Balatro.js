...
$('#deal').click(() => {
	...
	document.querySelector("footer").innerText += `  | AutoUpdate info: [ ${jokerKeys.length}/150 jokers ( ${100*(jokerKeys.length/150)}% ) | ${bosses}/30 blinds ( ${100*(bosses/30)}% ) ]`
...
function startBlind() {
	...
	$("#store-box").animate({opacity:0}, timing, "swing", ()=>{
...
function showBlind(){
	if(!alwaysShowShop){
		$("#store-box").animate({opacity:0}, 250, "swing", ()=>{
			document.getElementById('store-box').hidden = true;
			document.getElementById('card-table').hidden = false;
		});
	}
...
function fillShopUpperDeck(){
	...
	for (let index = 0; index < game.player.topShelfMax; index++) {
		...
		//Random
		let randomChoiceID = Math.ceil( Math.random() * ( topShelfChoices.length-1 ) )
		...
		let edition = RULEBOOK.editions.base;
		if(Math.floor(Math.random()*5) == 0){
			switch (Math.floor(Math.random()*4)) {
				case 0:
					edition = RULEBOOK.editions.foil;
					break;
				case 1:
					edition = RULEBOOK.editions.polychrome;
					break;
				case 2:
					edition = RULEBOOK.editions.holographic;
					break;
				case 3:
					edition = RULEBOOK.editions.negative
					break;
			}
		}
		// edition = RULEBOOK.editions.foil;
...
function buyPack(pack, uid, price){
	...
	packHand = new cards.Hand({ faceUp: true, y: 250 });
...
function buffoonPack(pack){
	...
	for (let index = 0; index < pack.options; index++) {
		...
		newPlanetElement.prop("title", chosenPlanet );
		newPlanetElement.click(()=>{
			game.player.addJoker(randomPlanet);
			newPlanetElement.hide();
			maxTake--;
			$("#boosterPackText").text(`Choose ${maxTake}`);
			if(maxTake == 0){
...
document.addEventListener("keydown", ev => {
	const keyActions = {
		...
		'i' : () => { if(sv_cheats){ game.player.addJoker(RULEBOOK.jokers[prompt("joker property")]) } },
...
$("button:not(.card,.joker)").on("click", ()=>{
...
function updateInfo() {
	...
	document.querySelector('#infoAnte').innerText = game.ante + 1
...
try {
		...
		$(satisfyingWrapper).animate({opacity: 1, apple: 1}, { duration: 400, step: ()=>{
...
async function countScore(elementId, target, speed){
	...
	while (toNumber(ele.innerText) != target) {
		...
		if( Math.abs(target - toNumber(ele.innerText) ) > 100 )   countingDivisor = 10;
		if( Math.abs(target - toNumber(ele.innerText) ) > 1000 )  countingDivisor = 100;
		if( Math.abs(target - toNumber(ele.innerText) ) > 10000 ) countingDivisor = 1000;
		if( Math.abs(target - toNumber(ele.innerText) ) > 50000 ) countingDivisor = 5000;

		
		if($("#instantCount").prop('checked') || ele.innerText < 0){
